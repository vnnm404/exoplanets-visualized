...
var y = d3.scaleRadial().range([innerRadius, outerRadius]).domain([0, 10000]);
...
svg
  ...
  .attr("text-anchor", function (d) {
    return (x(d.end) + x.bandwidth() / 2 + Math.PI) % (2 * Math.PI) < Math.PI
      ? "end"
      : "start";
  })
  .attr("transform", function (d) {
    return (
      "rotate(" +
      (((x(d.end) + x.bandwidth() / 2) * 180) / Math.PI - 90) +
      ")" +
      "translate(" +
      (y(d.value) + 10) +
      ",0)"
...
svg
  ...
  .on("mouseover", function (d) {
    ...
    d3.select(this).attr("opacity", "1"); // Modify fill color on hover
  })
  .on("mouseout", function (d) {
    // Add mouseout event listener
    d3.select(this).attr("opacity", "0.5"); // Reset fill color on mouseout
  });
...
svg2
  ...
  .join("g")
  .attr("text-anchor", function (d) {
    return (x2(d.end) + x2.bandwidth() / 2 + Math.PI) % (2 * Math.PI) < Math.PI
      ? "end"
      : "start";
  })
  .attr("transform", function (d) {
    return (
      "rotate(" +
      (((x2(d.end) + x2.bandwidth() / 2) * 180) / Math.PI - 90) +
      ")" +
      "translate(" +
      (y(d.value) + 10) +
      ",0)"
...
let yTick = yAxis.selectAll("g").data(y.ticks(3).slice(1)).enter().append("g");
...
yAxis = svg2.append("g").attr("text-anchor", "middle");

yTick = yAxis.selectAll("g").data(y.ticks(3).slice(1)).enter().append("g");
