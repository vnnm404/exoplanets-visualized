    ...
    this.select(".planetGroup")
      ...
      .each("end", function (d, i) {
        ...
        if (i === 0) {
          exo.planets
            .filter(function (d) {
              return d.type == "local" && d.name != "Sun";
            })
...
exo.radial = function () {
  return function () {
    ...
    d3.selectAll(".x, .y").each(function () {
      d3.select(this).transition().duration(2000).attr("opacity", "0"); //.delay(2000).remove();
    });
...
exo.graph = function (options) {
  ...
  if (exo.solar) {
    ...
    var yAxis = d3.svg
      .axis()
      .scale(exo.scaleYSolar[opt.yScale]())
      .ticks(10)
      .orient("left");
  } else {
    ...
    d3.select(this)
      ...
      .attr("transform", function (d) {
        ...
        if (exo.solar) {
          return (
            "translate(" +
            exo.scaleXSolar[opt.xScale]()(d[opt.xScale]) +
            "," +
            exo.scaleYSolar[opt.yScale]()(d[opt.yScale]) +
            ")"
          );
        } else {
          return (
            "translate(" +
            exo.scaleX[opt.xScale]()(d[opt.xScale]) +
            "," +
            exo.scaleY[opt.yScale]()(d[opt.yScale]) +
            ")"
          );
        }
      })
...
exo.createLabelGroup = function () {
  return function () {
    ...
    labelGroup.append("circle").attr("r", function (d) {
...
exo.calcPlanetTemp = function (tStar, rStar, axis) {
  if (tStar > 0 && rStar > 0 && axis > 0) {
    ...
    return (
      tStar *
      Math.pow((rStar * Math.pow((1 - albedo) / 0.7), 0.5) / (2 * axis), 0.5)
    );
  } else {
    return 0;
  }
...
exo.renderPlanets = function () {
  // join planet objects with initial (empty) data
  return function () {
    exo.planets = exo.planetStage
      .selectAll(".item")
      .data(exo.data, function (d) {
        return d.name;
      });
...
exo.scaleScale = function () {
  ...
  if (!exo.solar) {
    newData = exo.data.filter(function (d) {
      return d.type != "local";
    });
  }
  var filteredMax = d3.max(newData, function (d) {
    return d.axis;
  });
  var newWidth = d3.scale
    .linear()
    .domain([0, filteredMax])
    .range([0, $(window).width()]);
  //scale the entire dataset within the range of the new set
  return newWidth(
    d3.max(exo.dataSet, function (d) {
      return d.axis;
    })
  );
};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  d3.csv("data/NasaExoplanetArchive.csv", function (data) {
    ...
    document
      .getElementById("solarSystem")
      .addEventListener("click", function () {
        resetZoom();
        exo.solar = true;
        exo.rDuration = 30000;
        exo.data = exo.dataSet.filter(function (d) {
          return d.type == "local";
        });
    ...
    exo.centerRadialStage = function (stage) {
      stage.attr(
        "transform",
        "translate(" + exo.width / 2 + "," + exo.height / 2 + ")"
      );
    };
    ...
    exo.renderPlanets = function () {
      exo.planets = exo.planetStage
        ...
        .attr("class", "planets")
        .attr("r", function (d) {
          return exo.scaleRadial(d.radius);
        })
        .attr("cx", 0)
        .attr("cy", 0)
        .attr("fill", function (d) {
          return exo.color(d.type);
        })
        .attr("stroke", "white")
        .attr("stroke-width", 1)
        .style("opacity", function (d) {
          return exo.scaleOpacity(d.period);
        })
        .transition()
        .duration(exo.rDuration)
    ...
    exo.graph = function (options) {
      ...
      xScale = d3.scale
        .linear()
        .domain([
          d3.min(exo.data, function (d) {
            return d[options.xScale];
          }),
          d3.max(exo.data, function (d) {
            return d[options.xScale];
          }),
      ...
      stage
        ...
        .call(xAxis);
      stage.append("g").attr("class", "axis").call(yAxis);
      stage
        .append("text")
        ...
        .attr(
          "transform",
          "translate(" + exo.width / 2 + "," + (exo.height + 40) + ")"
        )
        .text(options.xLabel);
      stage
        .append("text")
      ...
      stage
        .append("g")
        .attr("class", "grid")
        .call(yAxis.tickSize(-exo.width, 0, 0).tickFormat(""));
      stage
        .append("g")
        .attr("class", "grid")
        .call(xAxis.tickSize(-exo.height, 0, 0).tickFormat(""));
      shapes
        .transition()
        .duration(exo.tDuration)
        .attr("cx", function (d) {
          return xScale(d[options.xScale]);
        })
        .attr("cy", function (d) {
          return yScale(d[options.yScale]);
        });
    };
  })(d3);
});
